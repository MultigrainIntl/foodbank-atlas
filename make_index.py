...
<div class="wrap">
 ...
 <p>Every food bank's map is generated by the same pipeline from the same Census data — same method, same look, same scoring — so every map is built and read the same way, and adding a new food bank is a one-line change. The <em>scores</em> are a different matter: each map's scale is stretched to its own service area, so a score ranks neighbourhoods within one food bank's region and is not comparable with a score on another food bank's map.</p>
